...
SESSION="${SESSION_NAME:-model_zoo_wandb}"
SWEEP_YAML="${SWEEP_YAML:-sweeps/cifar10_default_wandb_sweep.yaml}"
...
launch_agent_window() {
  ...
  local tmux_cmd="cd \"${ROOT_DIR}\" && CUDA_VISIBLE_DEVICES=${gpu} OMP_NUM_THREADS=\${OMP_NUM_THREADS:-1} PYTHONPATH=\"${ROOT_DIR}:${ROOT_DIR}/graph_metanetworks-main\" wandb agent \"${sweep_target}\" 2>&1 | tee -a \"${log_file}\""
...
create_sweep() {
  local output
  output="$(cd "${ROOT_DIR}" && PYTHONPATH="${ROOT_DIR}:${ROOT_DIR}/graph_metanetworks-main" wandb sweep "${SWEEP_YAML}")"
  echo "${output}" | tee -a "${MASTER_LOG}" >/dev/null
  local sweep_target
  # W&B prints: "wandb: Run sweep agent with: wandb agent entity/project/id"
